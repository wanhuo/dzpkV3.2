...
static HandleDownLoad* g_asyncImg =  nullptr;

HandleDownLoad* HandleDownLoad::getInstance()
{
	if (!g_asyncImg)
	{
		g_asyncImg = new HandleDownLoad();
	}
	return g_asyncImg;
}

void HandleDownLoad::destoryInstance()
{
	if (g_asyncImg)
	{
		delete g_asyncImg;
...
void HandleDownLoad::getImage(const std::string& strUrl)
{
	HttpRequest* request = new HttpRequest();
	std::string strURLNew=std::string(DEFAULT_GOODS_URL) + strUrl;
	request->setUrl(strURLNew.c_str());
	request->setRequestType(HttpRequest::Type::GET);
	request->setResponseCallback(CC_CALLBACK_2(HandleDownLoad::onHttpRequestCompleted, HandleDownLoad::getInstance()));
	HttpClient::getInstance()->send(request);
	request->release();
}

void HandleDownLoad::onHttpRequestCompleted(cocos2d::network::HttpClient *sender, cocos2d::network::HttpResponse *response)
{
	if (!response)
	{
		return;
	...
	// update sprite image
	HandleDownLoadView *pLayer = m_data.at(strUrl);
	if (pLayer)
	{
